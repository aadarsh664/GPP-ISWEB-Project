...
type FocusRevealProps = {
  ...
  onComplete?: () => void;
  appearTrigger?: "immediate" | "custom" | "scroll";
...
const FocusReveal = ({
  ...
  const [isTriggered, setIsTriggered] = React.useState(appearTrigger === "immediate");
  ...
  useEffect(() => {
    if (appearTrigger === "custom" && customTriggerEvent) {
      const handleTrigger = () => setIsTriggered(true);
      window.addEventListener(customTriggerEvent, handleTrigger);
      return () => window.removeEventListener(customTriggerEvent, handleTrigger);
    } else if (appearTrigger === "scroll" && inView) {
      setIsTriggered(true);
    }
  }, [appearTrigger, customTriggerEvent, inView]);
  ...
  return (
    <MotionTag ref={containerRef} aria-label={text} className={className} style={rootStyle}>
