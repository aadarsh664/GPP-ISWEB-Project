...
function __OriginkitBase_SmokyText(__props: any) {
    const {
        text = "SMOKY\nTEXT",
        font = {} as any,
        color = "whitesmoke",
        appearTrigger = "default" as "default" | "hover" | "scroll" | "custom",
        customTriggerEvent = "",
        scrollConfig = {} as any,
    ...
    useEffect(() => {
        ...
        if (appearTrigger === "custom" && customTriggerEvent) {
            setPhase("hidden")
            const onCustomEvent = () => runAppear()
            window.addEventListener(customTriggerEvent, onCustomEvent)
            return () => {
                window.removeEventListener(customTriggerEvent, onCustomEvent)
                clear()
            }
        }
    ...
    const textAlign = (fontAny?.textAlign ?? "left") as string
    const justify =
        textAlign === "right"
            ? "flex-end"
            : textAlign === "center"
...
export default function SmokyText(props: Record<string, unknown>) {
  return <__OriginkitBase_SmokyText {...COMPONENT_DEFAULTS} {...props} />;
}
