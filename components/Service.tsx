  ...
  return (
    ...
    >
      <motion.div
        ...
        className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-[4vw] items-center"
      >
        ...
        <div className="z-10 flex flex-col justify-center h-full py-4 md:py-0 order-1 min-w-0">
          ...
          <h2 className="font-black mb-4 md:mb-6 leading-[0.9] text-slate-900 tracking-tight uppercase" style={{ fontSize: 'clamp(2rem, 5vw, 6rem)' }}>{service.title}</h2>
          ...
          >
            {service.brands.map((brand) => (
              <button
                ...
                className={`flex items-center gap-3 w-full text-left p-[clamp(0.5rem,1vw,0.75rem)] rounded-xl transition-all ${activeBrand === brand ? 'bg-black text-white scale-[1.02] shadow-xl' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'
                  }`}
              >
        ...
        <div
          className="relative w-full rounded-[32px] md:rounded-[40px] overflow-hidden group order-2 mt-6 md:mt-0 h-[350px] md:h-[clamp(400px,60vh,700px)] isolation-isolate transform-gpu"
...
const Service: React.FC = () => {
  return (
    <section id="service" className="contain-section relative bg-white pb-24">
